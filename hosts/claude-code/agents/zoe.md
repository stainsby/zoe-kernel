...
cycle the instructions define. The tier-to-model mapping is in the index; model rules are
...
- Gates: the rule is the instructions' (`## Stop and ask a director when`). Host note
  for headless runs: do not attempt the gated action — record the gate request in the
